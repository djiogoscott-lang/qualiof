...
export function Skeleton({ className }: { className?: string }) {
  return <div className={cn('animate-pulse bg-white/[0.06] rounded-md', className)} />;
}
...
export function KpiPillSkeleton() {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft backdrop-blur-md p-5 flex items-center gap-4">
...
export function ListSkeleton({ rows = 6 }: { rows?: number }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft backdrop-blur-md overflow-hidden">
...
export function CardSkeleton({ lines = 4, className }: { lines?: number; className?: string }) {
  return (
    <div className={cn('rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft p-6 space-y-3', className)}>
...
export function DataTableSkeleton({ rows = 8, columns = 4 }: { rows?: number; columns?: number }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft backdrop-blur-md overflow-hidden">
      {/* Header */}
      <div className="bg-white/[0.02] border-b border-white/5 px-4 py-3.5 flex gap-4">
      ...
      {Array.from({ length: rows }).map((_, r) => (
        <div key={r} className="border-b border-white/5 last:border-0 px-4 py-3.5 flex items-center gap-4">
...
export function DetailSectionSkeleton({ rows = 4, title = true }: { rows?: number; title?: boolean }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft backdrop-blur-md p-6 space-y-4">
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-soft backdrop-blur-md p-5 space-y-4">
