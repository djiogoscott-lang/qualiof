...
export default async function DossiersOpcoPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  function renderRow(r: RowItem, idx: number) {
    ...
    return (
      <tr
        key={r.id}
        className={`border-b border-slate-200 last:border-0 transition-colors ${rowBg}`}
      >
        <td className="px-3 py-2"><DossierRowCheckbox id={r.id} /></td>
        <td className="px-3 py-2 whitespace-nowrap text-xs">
          <Link href={`/app/sessions/${r.session.id}`} className="text-slate-900 hover:text-primary">
        ...
        <td className="px-3 py-2">
          ...
          {r.sponsorOrg?.network && r.sponsorOrg.network !== r.sponsorOrg.legalName && (
            <div className="text-[10px] text-slate-500">via {r.sponsorOrg.network}</div>
          )}
        </td>
        <td className="px-3 py-2">
          <Link
  ...
  return (
    ...
    <div className="space-y-6">
          ...
          <div className="mt-4 pt-4 border-t border-primary-200/50">
            <div className="flex items-center justify-between text-xs mb-1.5">
              <div className="text-slate-500">
                <strong className="text-slate-900">Objectif {new Date().toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}</strong>
                {' · '}encaissé ce mois
              </div>
              <div className="tabular-nums">
                <strong className="text-slate-900">{fmtEUR.format(monthlyCollected)}</strong>
                <span className="text-slate-500"> / {fmtEUR.format(monthlyTarget)}</span>
                <span className={`ml-2 font-semibold ${monthlyPct >= 100 ? 'text-emerald-700' : monthlyPct >= 70 ? 'text-amber-700' : 'text-slate-500'}`}>
                  {monthlyPct}%
                </span>
              </div>
            </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <a
          href={exportUrl}
          className="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-md border border-slate-200 bg-white text-sm font-medium hover:bg-slate-100/40 transition-colors"
      ...
      {/* Tableau */}
      <section className="rounded-2xl border border-slate-200 bg-white overflow-hidden -mx-4 sm:mx-0">
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                <>
                  ...
                  {ungrouped.length > 0 && (
                    <>
                      <tr className="bg-slate-100/20 border-t-2 border-slate-200">
                        <td colSpan={8} className="px-3 py-1.5 text-[10px] uppercase tracking-wide text-slate-500">
            ...
            {rows.length > 0 && (
              <tfoot>
                <tr className="bg-slate-100/40 font-medium">
                  <td colSpan={5} className="px-3 py-2 text-xs uppercase tracking-wide text-slate-500">
...
function Th({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <th
      className={`px-3 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap ${className ?? ''}`}
    >
      {children}
    </th>
  );
